...
def main():
    path = (input("Enter the path to the directory you want to enumerate: "))
    depth = int(input("Enter the depth of the directory to enumerate (default is 0): "))
    path = clean_path(path)
    files = _get_directory_contents(path, depth)
    m = get_all_metadata(files)
    for key in m.keys():
        value = m[key]
...
if __name__ == "__main__":
    ...
    """_ = input("Enter file path: ")
    _ = clean_path(_)
    files = _get_directory_contents(_, 2)
    ...
    _ = input("Enter file path: ")
    _ = clean_path(_)
    mClean = CleanFile(name = os.path.basename(_), path = _)
    print(mClean.Metadata)
    m = mClean.Metadata_dict
    m["TITLE"] = "testTitle"
    mClean.Metadata_dict = m
    mClean.Name = "testName"
    #mClean.Save()
